...
void ENEMIES::collision() {
	...
	for (int j = Enemy.curNum - 1; j >= 0; j--) {
		if (Enemies[j].invincbleRestTime > 0) {
			Enemies[j].invincbleRestTime -= delta;
		}
		else {
			Enemies[j].color = Enemy.nomalColor;
			for (int i = bullets->curNum() - 1; i >= 0; i--) {
				VECTOR2 vec = Enemies[j].pos - bullets->pos(i);
				if (sqLength(vec) < sqDistance) {
					Enemies[j].hp--;
					Enemies[j].invincbleRestTime = Enemy.invincbleTime;
...
void ENEMIES::draw(){
	for (int i = 0; i < Enemy.curNum; i++) {
		...
		game()->hpgauge()->draw(Enemies[i].pos,
			Enemy.hpGaugeOfst, Enemies[i].hp);
	}
} 